...
class AuthServiceClient:
    ...
    def __init__(self, base_url: str, token_url: str):
        ...
        logger.info(f"Initialized auth service client with validation URL: {self.validation_url}")
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                if response.status_code == status.HTTP_401_UNAUTHORIZED:
                    logger.warning(f"Token validation failed: Unauthorized - {response.text}")
                    raise HTTPException(
                        status_code=status.HTTP_401_UNAUTHORIZED,
                ...
                logger.error(f"Token validation failed with status {response.status_code} - Response: {response.text}")
                raise HTTPException(
