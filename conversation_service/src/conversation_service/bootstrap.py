"""
Bootstrap module for Conversation Service.
...
CLIENT_NAME = "conversation-service-client"
CLIENT_DESCRIPTION = (
    "M2M client for the conversation service. Auto-generated by bootstrap."
)
...
def update_env_file(client_id: str, client_secret: str) -> bool:
    ...
    if not env_file_path.exists():
        ...
        os.environ["CONVERSATION_SERVICE_CLIENT_ID"] = client_id
        os.environ["CONVERSATION_SERVICE_CLIENT_SECRET"] = client_secret
        return False

    try:
        set_key(str(env_file_path), "CONVERSATION_SERVICE_CLIENT_ID", client_id)
        set_key(str(env_file_path), "CONVERSATION_SERVICE_CLIENT_SECRET", client_secret)
        os.environ["CONVERSATION_SERVICE_CLIENT_ID"] = client_id
        os.environ["CONVERSATION_SERVICE_CLIENT_SECRET"] = client_secret
...
async def run_bootstrap() -> bool:
    """
    Main bootstrap function for Conversation Service.
    Sets up M2M credentials with the auth service.
    """
    logger.info("Starting Conversation Service bootstrap process")

    client_id = os.getenv("CONVERSATION_SERVICE_CLIENT_ID")
    client_secret = os.getenv("CONVERSATION_SERVICE_CLIENT_SECRET")
    ...
    logger.info("Conversation Service bootstrap completed successfully")
    return True
