...
from auth_service.rate_limiting import TOKEN_LIMIT, limiter
from auth_service.schemas.app_client_schemas import (
    AccessTokenResponse,
    AppClientTokenRequest,
)
...
from ..dependencies.user_deps import get_current_supabase_user
...
async def get_client_token(
    ...
    if not verify_client_secret(token_request.client_secret, client.client_secret_hash):
        logger.warning(
            f"Invalid client secret for client ID '{token_request.client_id}'"
        )
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Invalid client credentials.",
    ...
    role_query = (
        select(Role)
        .join(AppClientRole, Role.id == AppClientRole.role_id)
    ...
    )  # Convert to seconds for the response
