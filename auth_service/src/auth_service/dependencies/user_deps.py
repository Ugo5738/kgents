...
oauth2_scheme = OAuth2PasswordBearer(
    tokenUrl="/api/v1/auth/users/login"
)  # TODO: change to /auth/users/login if not working as expected
...
async def get_current_supabase_user(
    ...
    try:
        logger.debug(f"Attempting to get user with token: {token[:20]}...")
        ...
        # We parse the response into our Pydantic model for type safety
        return SupabaseUser.model_validate(user_response.user)
