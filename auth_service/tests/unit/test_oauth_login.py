...
async def test_oauth_login_initiate_google(client: AsyncClient, mock_supabase_client):
    ...
    mock_supabase_client.auth.sign_in_with_oauth = AsyncMock(
...
async def test_oauth_callback_success(
    client: AsyncClient, mock_supabase_client, db_session
):
    ...
    mock_session.model_dump = MagicMock(
        return_value={"access_token": "mock_token", "user": {"id": mock_user_id}}
    )
    ...
    existing_user = await profile_crud.get_profile_by_user_id(db_session, mock_user_id)
    if existing_user:
    ...
    if response.status_code == status.HTTP_200_OK:
        ...
        db_user = await profile_crud.get_profile_by_user_id(db_session, mock_user_id)
        if db_user:
