...
def copy_auth_schema(source_db, target_db, host, port, user, password):
    ...
    try:
        ...
        source_conn = psycopg.connect(
            host=host, port=port, user=user, password=password, dbname=source_db
        )
        ...
        with source_conn.cursor() as cursor:
            ...
            cursor.execute(
                "SELECT pg_catalog.pg_get_userbyid(nspowner) as owner FROM pg_catalog.pg_namespace WHERE nspname = 'auth';"
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Copy auth schema from source to target database"
    )
    parser.add_argument(
        "--source-db",
        default="postgres",
        help="Source database with auth schema (default: postgres)",
    )
    parser.add_argument(
        "--target-db",
        default="auth_test_db",
        help="Target database for auth schema (default: auth_test_db)",
    )
    parser.add_argument(
        "--host",
        default=os.environ.get("DB_HOST", "supabase_db_kgents"),
        help="Database host",
    )
    parser.add_argument(
    ...
    success = copy_auth_schema(
        args.source_db, args.target_db, args.host, args.port, args.user, args.password
    )
