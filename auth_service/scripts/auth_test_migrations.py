...
AUTH_SERVICE_ROOT = PROJECT_ROOT  # No subdirectory in Docker containerkgents

# Get environment variables for database connection
DB_URL = os.environ.get(
    "DATABASE_URL",
    "postgresql+psycopg://postgres:postgres@supabase_db_kgents:5432/auth_test_db",
)
DB_HOST = os.environ.get("DB_HOST", "supabase_db_kgents")
...
def create_test_database():
    ...
    try:
        ...
        logger.info(
            f"Connecting to default postgres database to create test database if needed"
        )
...
async def verify_database_connection():
    ...
    try:
        ...
        logger.info(
            f"Verifying connection to test database: {DB_HOST}:{DB_PORT}/{DB_NAME}"
        )
...
def run_alembic_migrations():
    ...
    logger.info(
        "Skipping Alembic migrations and using direct SQL approach for more reliable setup"
    )
    ...
    if os.path.exists(alembic_ini_path):
        ...
        updated_content = re.sub(
            r"sqlalchemy\.url = .*", f"sqlalchemy.url = {clean_url}", config_content
        )
        ...
        logger.warning(
            f"alembic.ini not found at {alembic_ini_path}, using environment variables only"
        )
    ...
    try:
        ...
        if process.returncode != 0:
            ...
            logger.info(
                f"Current migration state: {process.stdout.strip() or 'No migrations applied'}"
            )
    except Exception as e:
...
def run_migrations_with_direct_psycopg():
    ...
    try:
        ...
        logger.info(
            "*** FALLBACK METHOD ACTIVATED: Creating tables directly with SQL ***"
        )
        ...
        try:
            with conn.cursor() as cursor:
                ...
                cursor.execute(
                    "CREATE INDEX IF NOT EXISTS ix_app_clients_client_name ON app_clients (client_name)"
                )
                ...
                cursor.execute(
                    "CREATE INDEX IF NOT EXISTS ix_app_client_refresh_tokens_app_client_id ON app_client_refresh_tokens (app_client_id)"
                )
                cursor.execute(
                    "CREATE INDEX IF NOT EXISTS ix_app_client_refresh_tokens_token_hash ON app_client_refresh_tokens (token_hash)"
                )
                ...
                cursor.execute(
                    "CREATE INDEX IF NOT EXISTS ix_profiles_username ON profiles (username)"
                )
                ...
                logger.info(
                    f"Profiles table exists after SQL creation: {profile_exists}"
                )
                ...
                if not profile_exists:
                    ...
                    logger.info(
                        "Made additional attempt to create profiles table with explicit schema"
                    )
...
async def main():
    ...
    logger.info(
        f"===== Test Database Fix Process Completed in {execution_time:.2f} seconds ====="
    )
